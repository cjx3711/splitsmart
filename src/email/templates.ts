...
export function resetPasswordEmail(input: ResetPasswordEmail): {
  subject: string;
  htmlBody: string;
  textBody: string;
} {
  const { name, resetUrl, expiresInHours } = input;
...
Someone asked to reset the password for your SplitSmart account. Open this link to choose a new one:
...
<html>
  <body style="margin:0;padding:24px;background:#f6f7f9;font-family:ui-sans-serif,system-ui,-apple-system,'Segoe UI',sans-serif;color:#16191d;line-height:1.5;">
    <div style="max-width:520px;margin:0 auto;background:#ffffff;border:1px solid #dfe3e8;border-radius:8px;padding:32px;">
      ...
      <p style="margin:0 0 16px;">Hi ${escapeHtml(name)},</p>
      <p style="margin:0 0 24px;">
        Someone asked to reset the password for your SplitSmart account. Open
